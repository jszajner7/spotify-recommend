...
//first API call to get artist ID by name search//
...
var getTopTracks = function(relID) {
    var emitter = new events.EventEmitter();
    unirest.get('https://api.spotify.com/v1/artist/' + relID + '/top-tracks?country=us')
        .end(function(response) {
            if (response.ok) {
                emitter.emit('end', response.body);
            }
...
app.get('/search/:name', function(req,res) {
    ...
    var searchReq = getFromAPI('search', {
      ...
      searchReq.on('end', function(item) {
          ...
          //get the arists and ID for use in next call
          ...
          relatedArtist.on('end', function(item) {
              ...
              //set the related artist to make the html work//
              artist.related = item.artist;
...
//expreww http listener//
app.listen(prosses.env.PORT || 8080);
